...
import { getSavedProducts } from '../savedproduct/api/actions'
...
class Offers extends PureComponent {
  ...
  static fetchData({ store }) {
    return store.dispatch(getProductList(1))
  }
    ...
    this.props.getSavedProducts()
      .then(response => {
        if (response.data.errors && response.data.errors.length > 0) {
          ...
          this.setState({
            savedProducts: response.data.data.userSavedProducts
          })
  ...
  isProductSaved=(id)=>{
    if(this.state.savedProducts!=undefined && this.state.savedProducts.length>0){
    let b=false;
        this.state.savedProducts.map(svproduct=>{
          if(svproduct.id==id)
      {   b= true
      console.log("saved"+b)
      }
  ...
  onClickLoadMore = () => {
    ...
    this.props.getMoreProducts(productId, 1)
  ...
  render() {
    ...
    return (
      <div>
        ...
        {/* SEO */}
        <Helmet>
          <title>La centrale Offers</title>
        </Helmet>

        {/* Top title bar */}
        <Grid style={{ backgroundColor: grey }} >
          <GridCell  alignCenter={true} style={{ padding: '2em',  textAlign: 'center'  }}>
            <H3 font="secondary">Offers</H3>

            <p style={{ marginTop: '1em', color: grey2 }}>Watch this space to keep updated with latest Offers.</p>
          </GridCell>
        </Grid>
              ...
              : list && list.length > 0
                ...
                list.map((product, index) => (
                  ...
                  <GridCell alignCenter={true} key={index} style={{ padding: '2em' }}>
                    ...
                    <ProductItem product={product} saved={this.isProductSaved(product.id)}/>
                  </GridCell>
...
Offers.propTypes = {
  ...
  getSavedProducts: PropTypes.func.isRequired,
}

// Component State
function OffersState(state) {
  return {
          user: state.user,
...
export default connect(OffersState, {getSavedProducts, getProductList, getMoreProducts, messageShow})(Offers)
